# -*- coding: utf-8 -*-
"""
Created on Wed Mar 12 21:26:53 2025
...
theta = np.pi / 3  # Mixing angle
phi_values = np.linspace(0, 2 * np.pi, 100)  # Range of phi values
...
for phi_idx, phi in enumerate(phi_values):
    ...
    for p_idx, p in enumerate(momentum):
        ...
        if len(saddle_points) >= 1:
            e_field_A[p_idx, phi_idx] = E_01 * np.sin(omega_1 * np.real(saddle_points[0])) + E_02 * np.sin(omega_2 * np.real(saddle_points[0]) + phi)

# Create a meshgrid for p and phi
P, PHI = np.meshgrid(momentum, phi_values, indexing='ij')

# Create contour plot
plt.figure(figsize=(10, 6))
contour = plt.contourf(P, PHI, e_field_A, levels=50, cmap='viridis')
plt.colorbar(contour, label="Electric Field (E)")

# Set labels
plt.xlabel("Momentum (p)")
plt.ylabel("Phase (phi)")
plt.title("Electric Field at Saddle Point A")
